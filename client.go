...
// AgentV2 adds support context-aware operations and session binding
// constraints.
//
// Unlike the Agent interface, AgentV2 methods accept a context.Context and a
// *Session, allowing the agent to enforce restrictions based on the current
// connection topology (e.g., preventing key usage if the session bindings do
// not match the key's constraints).
...
// AddedKey is an alias of InputKey, for backward compatibility.
type AddedKey = InputKey
...
type agentV1Adapter struct {
	agent AgentV2
}

func (a *agentV1Adapter) List() ([]*Key, error) {
	return a.agent.List(context.Background(), nil)
}

func (a *agentV1Adapter) Sign(key ssh.PublicKey, data []byte) (*ssh.Signature, error) {
	return a.agent.Sign(context.Background(), key, data, nil)
}

func (a *agentV1Adapter) SignWithFlags(key ssh.PublicKey, data []byte, flags SignatureFlags) (*ssh.Signature, error) {
	return a.agent.Sign(context.Background(), key, data, &SignOptions{Flags: flags})
}

func (a *agentV1Adapter) Add(key AddedKey) error {
	return a.agent.Add(context.Background(), key, nil)
}

func (a *agentV1Adapter) Remove(key ssh.PublicKey) error {
	return a.agent.Remove(context.Background(), key, nil)
}

func (a *agentV1Adapter) RemoveAll() error {
	return a.agent.RemoveAll(context.Background(), nil)
}

func (a *agentV1Adapter) Lock(passphrase []byte) error {
	return a.agent.Lock(context.Background(), passphrase, nil)
}

func (a *agentV1Adapter) Unlock(passphrase []byte) error {
	return a.agent.Unlock(context.Background(), passphrase, nil)
}

func (a *agentV1Adapter) Signers() ([]ssh.Signer, error) {
	return a.agent.Signers(context.Background())
}

func (a *agentV1Adapter) Extension(extensionType string, contents []byte) ([]byte, error) {
	return a.agent.Extension(context.Background(), extensionType, contents, nil)
}

type agentV2Adapter struct {
	agent Agent
}

func (a *agentV2Adapter) List(ctx context.Context, session *Session) ([]*Key, error) {
	return a.agent.List()
}

func (a *agentV2Adapter) Sign(ctx context.Context, key ssh.PublicKey, data []byte, options *SignOptions) (*ssh.Signature, error) {
	var flags SignatureFlags
	if options != nil {
		flags = options.Flags
	}

	if ext, ok := a.agent.(ExtendedAgent); ok {
		return ext.SignWithFlags(key, data, flags)
	}
	return a.agent.Sign(key, data)
}

func (a *agentV2Adapter) Add(ctx context.Context, key InputKey, session *Session) error {
	return a.agent.Add(key)
}

func (a *agentV2Adapter) Remove(ctx context.Context, key ssh.PublicKey, session *Session) error {
	return a.agent.Remove(key)
}

func (a *agentV2Adapter) RemoveAll(ctx context.Context, session *Session) error {
	return a.agent.RemoveAll()
}

func (a *agentV2Adapter) Lock(ctx context.Context, passphrase []byte, session *Session) error {
	return a.agent.Lock(passphrase)
}

func (a *agentV2Adapter) Unlock(ctx context.Context, passphrase []byte, session *Session) error {
	return a.agent.Unlock(passphrase)
}

func (a *agentV2Adapter) Signers(ctx context.Context) ([]ssh.Signer, error) {
	return a.agent.Signers()
}

func (a *agentV2Adapter) Extension(ctx context.Context, extensionType string, contents []byte, session *Session) ([]byte, error) {
	if ext, ok := a.agent.(ExtendedAgent); ok {
